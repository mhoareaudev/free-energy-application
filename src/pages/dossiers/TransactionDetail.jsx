...
export default function TransactionDetail({ transactionId, onBack, backLabel = 'Transactions' }) {
  ...
  const handleValidate = stageKey => {
    ...
    if (['dp', 'rac', 'vad'].includes(stageKey)) {
      const dpDone  = stageKey === 'dp'  ? true : (next.dp  || stageCompleted('dp',  cells, colMap, rowNum, sheetId))
      const racDone = stageKey === 'rac' ? true : (next.rac || stageCompleted('rac', cells, colMap, rowNum, sheetId))
      const vadDone = stageKey === 'vad' ? true : (next.vad || stageCompleted('vad', cells, colMap, rowNum, sheetId))
      if (dpDone && racDone && vadDone) sendPoseEligibleEmail()
    }
  }

  const sendPoseEligibleEmail = async () => {
    const appUrl          = import.meta.env.VITE_APP_URL || 'https://app.free-energy.re'
    const clientName      = nom || '—'
    const caName          = get('CHARGES_AFFAIRES') || ''
    const adresseComplete = [adresse, codePostal, ville].filter(Boolean).join(', ') || '—'

    // Destinataires : administratifs + administrateurs
    let recipients = []
    try {
      ...
      recipients = (admins || []).map(a => a.identifiant).filter(Boolean)
    } catch {}

    // Ajouter le chargé d'affaires s'il est identifiable
    if (caName) {
      try {
        const allProfiles = await supabaseGet('profiles', { select: 'prenom,nom,identifiant' })
        const caProfile = (allProfiles || []).find(p => {
          const fullName = [p.prenom, p.nom].filter(Boolean).join(' ')
          return fullName.toLowerCase().trim() === caName.toLowerCase().trim()
        })
        if (caProfile?.identifiant && !recipients.includes(caProfile.identifiant)) {
          recipients.push(caProfile.identifiant)
        }
      } catch {}
    }
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;padding:32px 16px;">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="background:#0f172a;padding:24px 32px;">
            ...
            <p style="margin:4px 0 0;font-size:13px;color:#94a3b8;">Éligibilité à la pose confirmée</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:32px;">
            ...
            <p style="margin:0 0 20px;font-size:15px;color:#374151;line-height:1.7;">
              Toutes les démarches administratives du dossier de
              <strong style="color:#0f172a;">${clientName}</strong>
              ont été complétées et validées.
              Ce client est <strong>éligible à la pose</strong> et peut être intégré au planning d'installation.
            </p>

            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr>
                <td style="background:#f0fdf4;border-left:4px solid #16a34a;border-radius:6px;padding:14px 16px;">
                  <p style="margin:0;font-size:14px;font-weight:700;color:#15803d;">✅ Démarches validées</p>
                  <p style="margin:8px 0 0;font-size:13.5px;color:#166534;line-height:1.8;">
                    &bull; <strong>DP</strong> — Déclaration préalable validée<br>
                    &bull; <strong>RAC</strong> — Raccordement EDF validé<br>
                    &bull; <strong>VAD</strong> — Validation administrative validée
                  </p>
                </td>
              </tr>
            </table>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr>
                <td style="background:#eff6ff;border-left:4px solid #3b82f6;border-radius:6px;padding:14px 16px;">
                  <p style="margin:0;font-size:14px;font-weight:700;color:#1d4ed8;">📅 Action requise — Planifier la pose</p>
                  <p style="margin:8px 0 0;font-size:13.5px;color:#1e3a8a;line-height:1.7;">
                    Merci de contacter le client pour convenir d'une date de pose et
                    l'intégrer au planning d'installation.
                  </p>
                </td>
              </tr>
            </table>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:8px;overflow:hidden;">
              ...
              <tr><td style="padding:14px 20px;">
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="padding:3px 0;font-size:13px;color:#64748b;width:180px;">Client</td>
                    <td style="padding:3px 0;font-size:13px;color:#1e293b;font-weight:600;">${clientName}</td>
                  </tr>
                  <tr>
                    <td style="padding:3px 0;font-size:13px;color:#64748b;">Adresse</td>
                    <td style="padding:3px 0;font-size:13px;color:#1e293b;">${adresseComplete}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td style="padding:3px 0;font-size:13px;color:#64748b;">Chargé d'affaires</td>
                    <td style="padding:3px 0;font-size:13px;color:#1e293b;">${caName || '—'}</td>
                  </tr>
                </table>
              </td></tr>
            </table>
        ...
        <tr>
          <td style="background:#f8fafc;padding:16px 32px;border-top:1px solid #e2e8f0;">
            <p style="margin:0;font-size:12px;color:#94a3b8;line-height:1.6;">
              Notification automatique — Application Free Energy. Ne pas répondre à cet email.
            </p>
          </td>
        </tr>
    ...
    try {
      await supabaseInvoke('send-email', {
        to: recipients,
        subject: `✅ Pose possible — ${clientName} est éligible à l'installation`,
        html,
      })
    } catch (e) {
      console.warn('Pose eligible email failed:', e)
    }
  }
